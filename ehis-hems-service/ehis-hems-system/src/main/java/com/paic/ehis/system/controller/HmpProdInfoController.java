...
import com.paic.ehis.system.service.IHmpProdServComService;
...
public class HmpProdInfoController extends BaseController {
    ...
    public TableDataInfo getServiceNetWorkList(HmpProdServCom hmpProdServCom)
    {
        startPage();
    ...
    public AjaxResult addProduct(@RequestBody ProductInfo product)
    {
        ...
        if(StringUtils.isEmpty(productInfo.getProductno())){
            ...
            //修改入库
            result = hmpProdInfoService.updateHmpProdInfo(productInfo);
        }
        ...
        //关联服务机构处理入库
        if(result > 0){
            //Object serviceOrgList = paramMap.get("serviceOrgList");
            List<HmpProdServCom> productServiceComSelects = product.getProdServComs();

            //Object serviceNetWorkList = paramMap.get("serviceNetWorkList");
            List<HmpComWebSite> comWebSites = product.getComWebSites();

            if(productServiceComSelects!=null && productServiceComSelects.size()==1){
                HmpProdServCom serviceComSelect = productServiceComSelects.get(0);
                ...
                if(StringUtils.isEmpty(serviceComSelect.getCurrency())){
                    serviceComSelect.setCurrency("CNY");//默认为CNY-人民币
                }
    ...
    public AjaxResult queryServiceOrgAndNetWork(@RequestBody String productNo)
    {
        ...
        if (!StringUtils.isEmpty(productNo))
        {
            ...
            ajax.put("serviceComs", productServiceComSelects);
