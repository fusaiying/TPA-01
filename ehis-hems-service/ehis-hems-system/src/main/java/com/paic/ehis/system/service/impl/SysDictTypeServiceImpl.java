...
public class SysDictTypeServiceImpl implements ISysDictTypeService {
    ...
    public List<SysDictsDataDTO> selectDictDataByTypeList(List<DictTypeVo> dictTypeVoList) {
        List<SysDictsDataDTO> tList = new ArrayList<SysDictsDataDTO>();
        if(dictTypeVoList != null && dictTypeVoList.size() > 0){
            for (DictTypeVo dictTypeVo: dictTypeVoList) {
                String dictType = dictTypeVo.getDictType();
                SysDictsDataDTO tSysDictsDataDTO = new SysDictsDataDTO();
                tSysDictsDataDTO.setDictType(dictType);
                List<SysDictData> dictDatas = DictUtils.getDictCache(dictType);
                if (StringUtils.isNotEmpty(dictDatas)) {
                    tSysDictsDataDTO.setDictDate(dictDatas);
                }else{
                    dictDatas = dictDataMapper.selectDictDataByType(dictType);
                    if (StringUtils.isNotEmpty(dictDatas)) {
                        DictUtils.setDictCache(dictType, dictDatas);
