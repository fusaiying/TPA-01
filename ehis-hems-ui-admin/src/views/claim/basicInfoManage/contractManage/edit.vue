  ...
  <div class="app-container">
    <el-card class="box-card" style="margin-top: 10px;">
     ...
     <el-form ref="searchForm" :model="searchForm" v-if="formTab" style="padding-bottom: 30px;margin-top:20px" label-width="130px" :rules="rules"
         ...
         <el-col :span="8">
           <el-form-item label="合约分类：" prop="contractsort">
             <el-select v-model="searchForm.contractsort" class="item-width" size="mini" placeholder="请选择">
               <el-option v-for="option in contractCategorys" :key="option.dictValue" :label="option.dictLabel" :value="option.dictValue" />
             </el-select>
           </el-form-item>
         </el-col>
       </el-row>
         ...
         <el-col :span="8">
          ...
          <el-col :span="8">
            <el-form-item label="提前协商天数：" prop="contractadvance">
              <el-input maxlength="4" oninput = "value=value.replace(/[^\d]/g,'')" :disabled="isShow" v-model="searchForm.contractadvance" class="item-width" clearable size="mini"
                        placeholder="请录入"/>
            </el-form-item>
          </el-col>
      ...
      <el-form ref="providerForm" :model="providerForm" v-if="!formTab"  style="padding-bottom: 30px;margin-top:20px" label-width="140px" :rules="providerRules"
               label-position="right" size="mini">
        <el-row>
          ...
          <el-col :span="8">
            <el-form-item label="押金金额：" prop="deposit">
              <el-input maxlength="14"  :disabled="isShow" v-model="providerForm.deposit" class="item-width" @input="changePrice('deposit')" clearable size="mini"
                        placeholder="请录入"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="15">
            <el-form-item  prop="expiryDate" key="expiryDate">
              <el-date-picker
                key="ff"
          ...
          <el-col :span="16" v-if="distcoteItem">
            <el-form-item label="诊疗费折扣：" prop="treatmentDiscount">
              <el-input maxlength="14" :disabled="isShow" v-model="providerForm.treatmentDiscount" @input="changePrice('treatmentDiscount')" class="item-width" clearable size="mini"
                        placeholder="请录入"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="8" v-if="countItem">
            <el-form-item label="次均控费：" prop="averageCost">
              <el-input  maxlength="14" :disabled="isShow" v-model="providerForm.averageCost" @input="changePrice('averageCost')" class="item-width" clearable size="mini"
                        placeholder="请录入"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="16" v-if="distcoteItem">
            <el-form-item label="检查费折扣：" prop="examineDiscount">
              <el-input maxlength="14" :disabled="isShow" v-model="providerForm.examineDiscount" @input="changePrice('examineDiscount')" class="item-width" clearable size="mini"
                        placeholder="请录入"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="16" v-if="distcoteItem">
            <el-form-item label="床位费折扣：" prop="bedDiscount">
              <el-input maxlength="14"  :disabled="isShow" v-model="providerForm.bedDiscount" @input="changePrice('bedDiscount')"  class="item-width" clearable size="mini"
                        placeholder="请录入"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="8" v-if="countItem">
            <el-form-item label="就诊次数：" prop="advicenum">
              <el-input  maxlength="4" :disabled="isShow" v-model="providerForm.advicenum" class="item-width" clearable size="mini"
                        placeholder="请录入"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="16" v-if="distcoteItem">
            <el-form-item label="护理费折扣：" prop="allowance">
              <el-input  maxlength="14" :disabled="isShow" v-model="providerForm.allowance" @input="changePrice('allowance')" class="item-width" clearable size="mini"
                         placeholder="请录入"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="16" v-if="distcoteItem">
            <el-form-item label="医药费折扣：" prop="costs">
              <el-input  maxlength="14" :disabled="isShow" v-model="providerForm.costs"  @input="changePrice('costs')" class="item-width" clearable size="mini"
                         placeholder="请录入"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="8">
            <el-form-item label="合作单位：" prop="cooperativeUnit">
              <el-select v-model="providerForm.cooperativeUnit" class="item-width" size="mini" placeholder="请选择">
                <el-option v-for="option in cooperativeUnitSelects" :key="option.dictValue" :label="option.dictLabel" :value="option.dictValue" />
              </el-select>
            </el-form-item>
          </el-col>

          <!-- 01=门诊，02=住院，03=体检，04=牙科，05=生育，06=眼科，07=疫苗 -->
          <el-col :span="16">
        ...
        <el-col :span="8">
          <el-form-item label="办公电话：" prop="tel">
            <el-input maxlength="11" :disabled="isShow" v-model="providerForm.tel" class="item-width" clearable size="mini"
                      placeholder="请录入"/>
          </el-form-item>
        </el-col>
      ...
      <el-table
        ...
        <el-table-column prop="serviceCode" label="服务项目名称" align="center">  <!-- serviceInfoSelects -->
          <template slot-scope="scope">
            <el-form-item v-if="scope.row.editing" :rules="serverInfoRules.serviceCode" :prop="'serverInfo.' + scope.$index + '.serviceCode'">
        ...
        <el-table-column prop="supplierCode" label="供应商名称" align="center">
          <template slot-scope="scope">
            <el-form-item v-if="scope.row.editing" :rules="serverInfoRules.supplierCode" :prop="'serverInfo.' + scope.$index + '.supplierCode'">
        ...
        <el-table-column label="服务项目价格" style="width: 500px" align="center">
          <template slot-scope="scope" >
            <el-form-item  :inline="true" v-if="scope.row.editing" :rules="serverInfoRules.minPrice" :prop="'serverInfo.' + scope.$index + '.minPrice'" >
            ...
            <el-form-item :inline="true" v-if="scope.row.editing" :rules="serverInfoRules.maxPrice" :prop="'serverInfo.' + scope.$index + '.maxPrice'" >
        ...
        <el-table-column prop="settleType" label="结算方式" align="center">
          <template slot-scope="scope">
            <el-form-item v-if="scope.row.editing" :rules="serverInfoRules.settleType" :prop="'serverInfo.' + scope.$index + '.settleType'">
              <el-select  style="width: 150px" size="mini" v-model="scope.row.settleType" class= "el-select item-width el-select--mini" placeholder="请选择">
                <el-option
        ...
        <el-table-column prop="settleCurrency" label="结算币种"  align="center"> <!-- currencys --> <!-- getCurrencyName -->
          <template slot-scope="scope">
            <el-form-item v-if="scope.row.editing" :rules="serverInfoRules.settleCurrency" :prop="'serverInfo.' + scope.$index + '.settleCurrency'">
              <el-select  style="width: 100px"  size="mini" v-model="scope.row.settleCurrency" class= "el-select item-width el-select--mini" placeholder="请选择">
        ...
        <el-table-column prop="limitnum" label="日限次数" style="width: 280px" align="center">
          <template slot-scope="scope" >
            <el-form-item v-if="scope.row.editing" :rules="serverInfoRules.limitnum" :prop="'serverInfo.' + scope.$index + '.limitnum'">
    ...
    data() {
      ...
      return {
        ...
        providerRules : {
          ...
          expiryDate: {trigger: ['change','blur'],validator: checkExpiryDate, required: true},
          contractPartyB: {trigger: ['change'], validator: checkcontractPartyB,required: true},
          endDate: {trigger: ['change','blur'], required: true, message: '合约终止日期'},
          treatmentDiscount: [{validator: checkTreatmentDiscount, required: true, trigger: 'blur'}],
          examineDiscount: [{validator: checkExamineDiscount, required: true, trigger: 'blur'}],
          bedDiscount: [{validator: checkBedDiscount, required: true, trigger: 'blur'}],
          allowance: [{validator: checkAllowance, required: true, trigger: 'blur'}],
          costs: [{validator: checkCosts, required: true, trigger: 'blur'}],
          averageCost: [{validator: checkAverageCost, required: true, trigger: 'blur'}],
          advicenum: [{validator: checkaDvicenum, required: true, trigger: 'blur'}],
          type: {trigger: ['change','blur'], validator: chectype, required: true, message: '次均控费类型必填'},
        },
        serverInfoRules: {
           ...
           maxPrice: [{required: true,validator: checkMaxPrice, trigger: 'blur'}],
        ...
        if(this.formTab) {
          ...
          this.$refs.providerForm.validate((valid) => {
            if (valid) {
              ...
              //检查  合约甲方 丙方
              if(!this.contractPartyASbu) {
                this.$message.warning('合约甲方已存在');
                return false;
              }
              if(!this.contractPartyCSub) {
                this.$message.warning('合约丙方已存在');
                return false;
              }
              if(!this.phoneSub) {
