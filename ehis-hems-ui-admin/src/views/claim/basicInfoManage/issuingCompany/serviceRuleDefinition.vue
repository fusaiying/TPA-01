  ...
  <div class="app-container">
    <el-card class="box-card">
      ...
      <el-form ref="searchForm" :model="formSearch" style="padding-bottom: 30px;" :rules="rules" label-width="110px"
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="出单公司名称：" prop="companycode ">
              <el-input v-model="formSearch.companycode" class="item-width" disabled clearable size="mini"
                        placeholder="平安寿险"/>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="产品名称：" prop="riskcode">
              <el-select multiple v-model="formSearch.riskcode" class="item-width" placeholder="请选择" clearable>
                <el-option v-for="item in riskName" :label="item.label" :value="item.value" :key="item.value"/>
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="8">
      ...
      <div>
        ...
        <el-table
          ...
          <el-table-column label="出单公司" prop="companycode" align="center"/>
          <el-table-column label="产品名称" prop="riskcode" align="center"/>
          <el-table-column label="结算类型" prop="accounttype" align="center">
          </el-table-column>
        ...
        <el-dialog
          ...
          title="提示"
          width="30%">
          <span>{{this.riskcode  + '该险种已维护,是否继续保存？' }}</span>
...
<script>
  import {listRule,getRule} from '@/api/baseInfo/issuingCompany.js'
  ...
  export default {
    data() {
      const checkServicefee = (rules, value, callback) => {
        ...
        const regx1 = /[^\d+\d-\.]/g
        if (value < 0) {
          callback(new Error("服务费不为正数，请检查"));
        } else if (!regx.test(value)) {
          callback(new Error("服务费最多保留两位小数，请检查"));
        } else {
          callback();
        }
      }
      const checkPremrate = (rules, value, callback) => {
        ...
        if (!regx.test(value)) {
          callback(new Error("保费比例不为0～100的正整数，请检查"));
        } else {
          callback();
        }
      }
      ...
      return {
        ...
        rules: {
          companycode: [{require: true, message: '出单公司不能为空', trigger: 'blur'}],
          riskcode: [{validator: checkRiskcode, trigger: 'blur'}],
          accounttype: [{require: true, message: '结算类型不能为空', trigger: 'blur'}],
