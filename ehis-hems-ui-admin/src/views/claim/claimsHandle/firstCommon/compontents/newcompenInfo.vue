  ...
  <el-card class="box-card" style="margin-top: 10px;">
    <el-collapse v-model="activeNames">
      <el-collapse-item  name="1" style="position: relative;">
        ...
        <span v-if="node==='complex'||node==='spotCheck'||node==='correct'" style="position: absolute;right: 40px;top: 0;">
          <el-button :disabled="status==='show' || disabledExemptModul" type="primary" size="mini" @click="openExemptModul">豁免({{exemptNum}})</el-button>
        </span>
        <el-form :disabled="(status==='show'||node==='complex'||node==='spotCheck'||node==='correct' || node ==='physical')?true:false" ref="baseForm" :rules="tableFormRules" :model="baseForm" size="mini">
          <span v-if="!(node==='complex'||node==='spotCheck'||node==='correct')" style="position: absolute;right: 40px;top: 0;">
            <el-button type="primary" size="mini" :disabled="disabledExemptModul" @click="openExemptModul">豁免({{exemptNum}})</el-button>
            <el-button type="primary" :loading="savePoilcyLoading" size="mini" @click="savePoilcy">赔付计算</el-button>
            <el-button type="primary" :loading="savePayConclusionLoading" size="mini" @click="savePayConclusion">保存</el-button>
          </span>
          <el-row>
            <el-table
              ref="table"
...
<!--                    <el-table-column align="center" prop="invoiceno" label="发票号" show-overflow-tooltip/>-->
                    ...
                    <el-table-column align="center" prop="invoicetype" :formatter="getinvoicetype" label="发票类型" show-overflow-tooltip/>
              ...
              <!-- <el-table-column label="不合理原因" width="100" prop="unreasonableexplain" align="center">
                <template slot-scope="scope">
                  <el-form-item :prop="'tableData.' + scope.$index + '.unreasonableexplain'"
                    ...
                    <el-input :disabled="scope.row.removeflag==='Y'" v-model="scope.row.unreasonableexplain" style="width: 90px" placeholder="请输入" size="mini"/>
                  </el-form-item>
                </template>
              </el-table-column> -->
              <el-table-column align="center" prop="deductible" label="免赔金额" >
              ...
              <el-table-column align="center" prop="finalamount" label="实赔金额" width="160">
                <template slot-scope="scope">
                  <el-form-item :prop="'tableData.' + scope.$index + '.finalamount'"
                    ...
                    <el-input-number v-model="scope.row.finalamount" :min="0" :precision="2" :step="0.01" style="width: 150px"  class="item_width" autocomplete="off" :disabled="scope.row.disabledFinalamount" placeholder="请输入" clearable />
                  </el-form-item>
              ...
              <el-table-column label="赔付结论" width="100" prop="conclusion" align="center">
                <template slot-scope="scope">
                  <el-form-item :prop="'tableData.' + scope.$index + '.conclusion'" :rules="tableFormRules.conclusion">
                    <el-select :disabled="scope.row.removeflag==='Y'" v-model="scope.row.conclusion" style="width: 90px;" placeholder="请选择" @change="changeRe(scope.row.conclusion,scope.$index)" clearable>
                      <el-option v-for="item in scope.row.conclusionList" :label="item.label" :value="item.value" :key="item.value"/>
                    </el-select>
                  </el-form-item>
                </template>
              </el-table-column>
              <el-table-column label="结论依据" width="100" prop="conclusionreason" align="center">
                <template slot-scope="scope">
                  <el-form-item :prop="'tableData.' + scope.$index + '.conclusionreason'" :rules="tableFormRules.conclusionreason">
                    <el-select :disabled="scope.row.removeflag==='Y' || scope.row.disabledCon == true" v-model="scope.row.conclusionreason" style="width: 90px;" placeholder="请选择" clearable @change="loadAll">
                      <el-option v-for="item in scope.row.reList" :label="item.label" :value="item.value" :key="item.value"/>
                    </el-select>
                  </el-form-item>
                </template>
              </el-table-column>
              ...
              <el-table-column label="赔付描述" width="200" prop="explanation" align="center">
                <template slot-scope="scope">
                  <el-form-item :prop="'tableData.' + scope.$index + '.explanation'" :rules="tableFormRules.explanation">
                    <el-autocomplete
              ...
              <el-table-column label="操作" fixed="right" align="center">
                <template slot-scope="scope">
                  <el-form-item>
                    <el-button type="text" v-if="scope.row.removeflag==='N'" @click="delGetDutyResult(scope.row, scope.$index, 'Y')">移 除</el-button>
                    <el-button type="text" v-if="scope.row.removeflag==='Y'" @click="delGetDutyResult(scope.row, scope.$index, 'N')">恢 复</el-button>
                  </el-form-item>
                </template>
              </el-table-column>
            </el-table>
    ...
    <exempt-modul v-model="exemptVis" :node="node" :claimno='claimno'></exempt-modul>
  </el-card>
</template>
<script>
import exemptModul from '../../common/modul/exempt'
import { delGetDutyResult, payCalculation, savePayConclusion, queryPolicyExemptionDetail , getTempalte, getContents ,queryAccordWithExemption,queryExemptionCount,
  deleteExemptionDetail, newCalculatePay, updateCalculate,savePolicyInfo
 ...
 // dicts: ['conclusion','conclusionreason','invoicetype','conclusionreason2','conclusionreason3','conclusionreason4','conclusionreason5'],
  ...
  data() {
    return {
      ...
      count: [],
      policyList: [],
      savePoilcyLoading: false,
      savePayConclusionLoading: false,
  ...
  mounted() {
    ...
    Bus.$on('updateIstpolicyListDisablsed',data =>{
      this.istpolicyListDisablsed = data
    })
    Bus.$on('updatePolicyList',data =>{
  ...
  methods: {
    ...
    savePayConclusion(){
      ...
      // if(this.calcFlag){
        ...
        this.savePayConclusionLoading = true
        let checkFlag = []
        checkIdx.map((item,index) => {
            ...
            'tableData.'+item.index+'.explanation','tableData.'+item.index+'.conclusionreason'],empty => {
              if(empty){
                this.savePayConclusionLoading = false
                checkFlag.push(0)
              }
            })
        })
        if(!checkFlag.includes(0)) {
          ...
          updateCalculate(this.baseForm.tableData).then(res => {
            if (res.status === '1') {
              this.savePayConclusionLoading = false
    ...
    savePoilcy() {
      this.savePoilcyLoading = true
      if (this.policyList && this.policyList.length == 0) {
        this.$message.error('请先勾选保单信息！')
        return false
      }
      let params = {}
      ...
      judgeBnf(this.claimno).then(response => {
        if (response.status === '1') {
          if(response.data !== '') {
            ...
            newCalculatePay(this.claimno).then(res => {
              if (res.status === '1') {
                this.savePoilcyLoading = false
                this.$message({ message: '操作成功！', type: 'success' })
