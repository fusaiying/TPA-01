  ...
  <el-card class="box-card" style="margin-top: 10px;">
    <el-collapse v-model="activeNames">
      <el-collapse-item name="1" style="position: relative;">
        <el-button type="primary" :disabled="status == 'show'" size="mini"  @click="savePayeeInfos" style="float:right;position:absolute;top:8px;right:30px">保存</el-button>
        <template slot="title">
...
<script>
import { savePayeeInfo,calculationPayAmount,savePayeeInfos } from "@/api/claim/handleDeal";
...
export default {
  ...
  methods: {
    ...
    getCollectedmodeOptions(data) {
      this.selectDictLabel(this.collectedmodeOptions,data)
    },
    ...
    savePayeeInfos() {
      this.$refs.payeeInfoForm.validate(valid => {
        if (valid) {
          let list = []
          this.payeeInfo.data.map((item) =>{
            let params = {}
            params.claimno = this.claimno,
            params.payeeno = item.payeeno,
            params.payoutratio =  item.payoutratio,
            params.payamount =  item.payamount,
            list.push(params)
          })
          savePayeeInfos(list).then(res =>{
            if (res.status == '1') {
              this.$message.success('领款人信息更新成功！')
              this.activeNames = []
            } else {
