      ...
      <el-collapse v-model="activeNames">
        <el-collapse-item title="材料信息" name="1" style="position: relative;">
          ...
          <span style="position: absolute;right: 40px;top: 0;">
            <el-button :disabled="(status==='show'?true:false) || acceptFlag" v-if="node==='review'||node==='complex'||node==='spotCheck'||node==='correct' " type="primary" size="mini" @click="fillingsHandle">补充物理件</el-button>
                    ...
                    <template slot-scope="scope">
                      <el-form-item v-if="scope.row.editFlag && scope.row.type" :prop="'data.' + (scope.$index * 2) + '.materialtype'" :rules="tableFormRules.materialtype">
                        <el-select v-model="scope.row.materialtype" placeholder="请选择" clearable
                          ...
                          <el-option v-for="item in material_typeOptions" :label="item.dictLabel" :value="item.dictValue" :key="item.dictValue"
...
<!--                  <el-table-column align="center" label="复印件数量" style="padding: 0" prop="copynum"></el-table-column>-->
                  <el-table-column min-width="150" align="center" label="材料备注" prop="remarks">-->
                    <template slot-scope="scope">
                      ...
                      <el-form-item v-if="node==='accept'" :prop="'data.' + scope.$index * 2 + '.remarks'" style="margin-bottom: 0">
...
<!--                  <el-table-column align="center" style="padding: 0" label="复印件数量" prop="copynum"></el-table-column>-->
                  ...
                  <el-table-column min-width="150" align="center" label="材料备注" prop="remarks">-->
                    <template slot-scope="scope">
                      ...
                      <!--                      </el-form-item>-->
                      <!--                      <span v-else>{{scope.row.remarks}}</span>-->
                      <el-form-item v-if="node==='accept'" :prop="'data.' +( scope.$index * 2 + 1) + '.remarks'" style="margin-bottom: 0">
                        <el-input type="textarea" maxlength="50" show-word-limit :rows="2" style="width: 90%;" v-model="scope.row.remarks" placeholder="请输入内容"></el-input>
                      </el-form-item>
                      <el-tooltip v-else class="item" effect="dark" :content="scope.row.remarks" placement="top-start">
                        <span class="show-little">{{scope.row.remarks}}</span>
...
<!--              </el-col>-->
          ...
          <fillsModul v-model="fillsDia" :dictList="dictList" :email="email" :tab-labels="tabLabels" :status="status" :node="node" :image-info="imageInfo" :claimno="claimno" type="02" @changeList="changeList" @imageUpdate="imageUpdate"/>
        </el-collapse-item>
      </el-collapse>
    </el-card>
        ...
        <el-collapse-item title="退件信息" name="1" style="position: relative;">
          ...
          <el-table
            ...
            <el-table-column property="materialtype" label="材料类型" align="center">
              <template slot-scope="scope">
                <span>{{scope.row.materialtype?getMaterial_typeOptions( scope.row.materialtype):''}}</span>
              </template>
            </el-table-column>
            ...
            <el-table-column property="refund" label="是否退件" align="center">
              <template slot-scope="scope">
                <span>{{scope.row.refund?getIs_refundOptions(scope.row.refund):'否'}}</span>
              </template>
            </el-table-column>
          </el-table>
        </el-collapse-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      getIs_refundOptions(data) {
        return this.selectDictLabel(this.is_refundOptions,data)
      },
      getMaterial_typeOptions(data) {
        return this.selectDictLabel(this.material_typeOptions,data)
      },
        ...
        materiaRefundInfo(this.claimno).then(res => {
          if (res.status === '1') {
            if(res.data){
              ...
              flag?this.$set(this.materialInfo, 'retux`rnChecked', true)
