        ...
        <el-form
          ...
          <el-table
            ...
            <el-table-column align="center" label="领款方式" prop="payeetype">
              <template slot-scope="scope">
                {{this.selectDictLabel(this.collectedmodeOptions, scope.row.payeetype) }}
              </template>
            </el-table-column>
            <el-table-column align="center" label="与被保人关系" prop="relationship" width="120">
              <template slot-scope="scope">
                {{this.selectDictLabel(this.relation_ship_applyOptions, scope.row.relationship) }}
              </template>
            </el-table-column>
            ...
            <el-table-column align="center" label="证件类型" prop="idtype">
              <template slot-scope="scope">
                {{this.selectDictLabel(this.applicant_idtypeOptions, scope.row.idtype)}}
              </template>
            </el-table-column>
            ...
            <el-table-column align="center" label="国籍" prop="nationality">
              <template slot-scope="scope">
                {{this.selectDictLabel(this.nativeplaceOptions, scope.row.nationality)}}
              </template>
            </el-table-column>
            <el-table-column align="center" label="职业" prop="occupation">
              <template slot-scope="scope">
                {{this.selectDictLabel(this.payee_occupationOptions, scope.row.occupation)}}
              </template>
            </el-table-column>
...
export default {
  ...
  mounted(){
    this.getDicts("applicant_idtype").then(response => {
      this.applicant_idtypeOptions = response.data;
    });
    this.getDicts("collectedmode").then(response => {
      this.collectedmodeOptions = response.data;
    });
    this.getDicts("relation_ship_apply").then(response => {
      this.relation_ship_applyOptions = response.data;
    });
