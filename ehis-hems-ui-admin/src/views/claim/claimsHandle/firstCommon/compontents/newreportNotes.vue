  ...
  <el-card class="box-card" style="margin-top: 10px;">
    <el-collapse v-model="activeNames">
      <el-collapse-item name="1">
        ...
        <el-form :disabled="status==='show'?true:false || acceptFlag || node ==='physical'" style="padding-bottom: 30px;" ref="tableForm" :model="caseRemark" :rules="tableFormRules"  size="mini" class="from_class_div">
          <el-row>
            <el-table
              ref="table"
              size="small"
              ...
              <el-table-column v-if="node!=='report'" label="备注类型" width="200px" prop="remarkstype" align="center">
                <template slot-scope="scope">
                  ...
                  <span v-else>{{selectDictLabel(remarks_typeOptions, scope.row.remarkstype)}}</span>
                </template>
              </el-table-column>
              <el-table-column label="备注内容" prop="remark" align="center">
                <template slot-scope="scope">
                  <el-form-item v-if="scope.row.editFlag"  :prop="'data.' + scope.$index + '.remark'" :rules="tableFormRules.remark">
                    <el-input maxlength="1000" :autosize="{ minRows: 2, maxRows: 4}" type="textarea"  show-word-limit v-model="scope.row.remark" placeholder="请输入" size="mini"/>
                  </el-form-item>
                  <el-form-item v-else>
                    <el-tooltip :content="scope.row.remark" placement="top" effect="light">
                      <p class="form-span" style="overflow: hidden;text-overflow:ellipsis;white-space: nowrap;">{{ scope.row.remark }}</p>
                    </el-tooltip>
                  </el-form-item>
              ...
              <el-table-column label="添加岗位" prop="casestep" width="200px"  align="center">
                <template slot-scope="scope">
                  <el-form-item>
                    <span class="form-span">{{selectDictLabel(tasknodeOptions, scope.row.casestep)}}</span>
                  </el-form-item>
                </template>
              </el-table-column>
              ...
              <el-table-column label="操作" width="120px"  align="center">
                <template slot-scope="scope">
                  <el-form-item>
                    <el-button v-if="scope.row.canedit == true && !scope.row.editFlag" type="text" @click="editCase(scope.$index,scope.row)">编辑</el-button>
                    <el-button v-if="scope.row.canedit == true" type="text" @click="dealCase(scope.$index,scope.row)">删除</el-button>
                    <el-button type="text" v-if="!scope.row.remarksno" @click="delHandle(scope.$index,scope.row)">取消</el-button>
                    <el-button type="text" v-if="scope.row.editFlag" @click="saveCaseRemark(scope.$index,scope.row)">保存</el-button>
                  </el-form-item>
                </template>
              </el-table-column>
            </el-table>
            <el-button :disabled="flag"  @click="openDia" size="small" style="margin-top:0.3%;font-size:14px;width: 100%;border-style: dashed"> + 添加</el-button>
          </el-row>
        </el-form>
      </el-collapse-item>
    </el-collapse>
    ...
    <el-dialog title="添加备注" :visible.sync="dialogVisible" append-to-body
      ...
      <el-form ref="diaForm" :model="form" label-width="100px">
        <el-form-item label="备注内容：" prop="remark"
          ...
          <el-input v-model="form.remark" maxlength="1000" :rows="5" show-word-limit style="width: 90%;" type="textarea" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
...
import { getCaseRemark, saveCaseRemark,deleteRemark } from "@/api/claim/handleDeal"
...
export default {
  ...
  methods: {
    getCasestepLabel(){
      let nodeLabel = this.dict.casestep.find((item) => {
    ...
    editCase(index,row) {
      this.$set(this.caseRemark.data[index],'editFlag',true)
      this.caseRemark.data = [...this.caseRemark.data]
      this.remarkno = row.remarkno
    },
    closeHandle(){
    ...
    dealCase(index,row) {
      deleteRemark(row.remarksno).then(res =>{
        if (res.status == '1') {
          this.$message.success('删除成功！')
          this.$emit('caseRemarkList')
        } else {
    ...
    saveCaseRemark(i, row){
      ...
      this.$refs.tableForm.validate((valid) => {
        if (valid) {
          ...
          let casestep = this.dict.casestep.find((item) => {
            return item.label === this.node
          })
          params.casestep =  casestep.value || ''
          if(this.node==='report') {
