      ...
      <el-collapse-item name="1" style="position: relative;">
        ...
        <el-card shadow="never" style="margin-bottom: 10px;">
          ...
          <el-tabs class="tabs-box" :stretch="true" v-model="activeName1" @tab-click="handleClick('1', activeName1)">
            <el-tab-pane v-for="tab in tabLabelss.originallist"
                         v-loading="loading1"
              ...
              <el-row v-if="imageInfos.originallist.list.length && activeName1===tab.name"
                      :id="'image' + tab.name + '1'" ref="image1"
                      class="imgList_div"
                      @scroll.native="getScrollTop('1', page0, tab.total, tab.name)">
                <el-col v-for="(url, i) in imageInfos.originallist.list" :span="3" :key="i" class="block">
                  <div v-if="node === 'accept' || node === 'report'" class="my-image-wrapper">
                    <el-image
                      ...
                      @click="getoriginallistPreviewList(i,url.pageid,imageInfos.originallist.list,'1')"/>
                    <el-button
        ...
        <el-card shadow="never" style="margin-bottom: 10px;" v-if="supplemenFlag">
          ...
          <el-tabs class="tabs-box" :stretch="true" v-model="activeName2" @tab-click="handleClick('2', activeName2)">
            <el-tab-pane v-for="tab in tabLabelss.supplementlist"
                         v-loading="loading2"
                         :label="`${tab.label}(${tab.total})`"
                         :name="tab.name" :key="tab.name">
              ...
              <el-row v-if="imageInfos.supplementlist.list.length && activeName2===tab.name" :id="'image' + tab.name + '2'" class="imgList_div" @scroll.native="getScrollTop('2', page1, tab.total, tab.name)">
                <el-col v-for="(url, i) in imageInfos.supplementlist.list" :span="3" :key="i" class="block">
                  <!--                  <els-image-->
                  <!--                    :src="'data:image/jpeg;base64,'+ url.src"-->
                  ...
                  <div v-if="node === 'accept' || node === 'report'" class="my-image-wrapper">
                    <el-image
                      ...
                      style="width: 96%;height: 160px"
                      fit="fill"
                      @click="getoriginallistPreviewList(i,url.pageid,imageInfos.supplementlist.list,'2')"
                    />
        ...
        <el-card shadow="never" style="margin-bottom: 10px;" v-if="surveyFlag">
          ...
          <el-tabs class="tabs-box" :stretch="true" v-model="activeName3" @tab-click="handleClick('3', activeName3)">
            <el-tab-pane v-for="tab in tabLabelss.surveylist"
                         v-loading="loading3"
                         :label="`${tab.label}(${tab.total})`"
                         :name="tab.name" :key="tab.name">
              ...
              <el-row v-if="imageInfos.surveylist.list.length && activeName3===tab.name" :id="'image' + tab.name + '3'" class="imgList_div"  @scroll.native="getScrollTop('3', page2, tab.total, tab.name)">
                <el-col v-for="(url, i) in imageInfos.surveylist.list" :span="3" :key="i" class="block">
                  <!--                  <el-image-->
                  <!--                    :src="'data:image/jpeg;base64,'+ url.src"-->
                  ...
                  <div v-if="node === 'accept' || node === 'report'" class="my-image-wrapper">
                    <el-image
                      ...
                      style="width: 96%;height: 160px"
                      fit="fill"
                      @click="getoriginallistPreviewList(i,url.pageid,imageInfos.surveylist.list,'3')"
                    />
                    <!-- :preview-src-list="['data:image/jpeg;base64,'+ url.src]" -->
                    <el-button
                      :disabled="status==='show'?true:acceptFlag || disableFlag"
        ...
        <el-card shadow="never" style="margin-bottom: 10px;" v-if="supplementpartFlag">
          ...
          <el-tabs class="tabs-box" :stretch="true" v-model="activeName4" @tab-click="handleClick('4', activeName4)">
            <el-tab-pane v-for="tab in tabLabelss.supplementpartlist"
                         v-loading="loading4"
                         :label="`${tab.label}(${tab.total})`"
                         :name="tab.name" :key="tab.name">
              ...
              <el-row v-if="imageInfos.supplementpartlist.list.length && activeName4===tab.name" :id="'image' + tab.name + '4'" class="imgList_div"  @scroll.native="getScrollTop('4', page3, tab.total, tab.name)">
                <el-col v-for="(url, i) in imageInfos.supplementpartlist.list" :span="3" :key="i" class="block">
                  <!--                  <el-image-->
                  <!--                    :src="'data:image/jpeg;base64,'+ url.src"-->
                  <!--                    style="width: 110px; height: 110px"-->
                  ...
                  <div v-if="node === 'accept' || node === 'report'" class="my-image-wrapper">
                    <el-image
                      ...
                      :preview-src-list="originallistPreviewList"
                      style="width: 96%;height: 160px"
                      @click="getoriginallistPreviewList(i,url.pageid,imageInfos.supplementpartlist.list,'4')"
                      fit="fill"/>
                    <!-- :preview-src-list="['data:image/jpeg;base64,'+ url.src]" -->
                    <el-button
                      :disabled="status==='show'?true:acceptFlag || disableFlag"
...
<script>
  import uploadModul from '../common/modul/upload'
  import { downloadData } from '@/utils'
  // import {encrypt} from "@/utils/rsaEncrypt"
  import {decrypt,encrypt } from "@/utils/rsaEncrypt"
  import { preView } from '@/api/claim/fileDeal'
  import { dowloadFileRar, claimDelete,getImageInfo,dowloadFileOne,getFileCount} from '@/api/claim/handleDeal'
  ...
  export default {
    ...
    created() {
      if (this.$route.query) {
        ...
        this.acceptFlag = this.querys.acceptFlag
    ...
    updated() {
      this.supplemenFlag = !!this.tabLabelss.supplementlist[0].total
      this.surveyFlag = !!this.tabLabelss.surveylist[0].total
      this.supplementpartFlag= !!this.tabLabelss.supplementpartlist[0].total
    },
    methods: {
      uploadHandle() {
        this.uploadVis = true
      ...
      getFileCount(){
        ...
        getFileCount(businessNo).then(res => {
          if (res.status === '1') {
            if(res.data){
              this.tabLabelss = res.data
            } else {
              for(let key in this.tabLabelss){
      ...
      changeList(type) {
        if(type==='1'){
          this.imageInfos.originallist.list = []
          this.imageInfos.originallist.previewList = []
        } else if(type==='2'){
          this.imageInfos.supplementlist.list = []
          this.imageInfos.supplementlist.previewList = []
        } else if(type==='3'){
          this.imageInfos.surveylist.list = []
          this.imageInfos.surveylist.previewList = []
        } else if(type==='4'){
          this.imageInfos.supplementpartlist.list =  []
          this.imageInfos.supplementpartlist.previewList = []
        }
      },
      // 影像文件
              ...
              if(params.type==='1'){
                ...
                if(response.data){
                  ...
                  response.data.list.map((item, i) => {
                    _this.imageInfos.originallist.list.push(item)
                    _this.imageInfos.originallist.previewList.push('data:image/jpeg;base64,'+ item.src)
                  })
                }
                ...
                if(response.data){
                  let _this = this
                  response.data.list.map((item, i) => {
                    _this.imageInfos.supplementlist.previewList.push('data:image/jpeg;base64,'+ item.src)
                    _this.imageInfos.supplementlist.list.push(item)
                  })
                }
                ...
                if(response.data){
                  let _this = this
                  response.data.list.map((item, i) => {
                    _this.imageInfos.surveylist.previewList.push('data:image/jpeg;base64,'+ item.src)
                    _this.imageInfos.surveylist.list.push(item)
                  })
                }
                ...
                if(response.data){
                  let _this = this
                  response.data.list.map((item, i) => {
                    _this.imageInfos.supplementpartlist.previewList.push('data:image/jpeg;base64,'+ item.src)
                    _this.imageInfos.supplementpartlist.list.push(item)
                    // _this.imageInfos.supplementpartlist.previewList = [..._this.imageInfo.supplementpartlist.previewList]
                    // _this.imageInfos.supplementpartlist.list = [..._this.imageInfo.supplementpartlist.list]
                  })
                }
              }
            } else {
