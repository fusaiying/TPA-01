    ...
    <div slot="header" class="clearfix">
      <div style="width: 100%;cursor: pointer;">
        ...
        <div style="float: right;">
          <el-button type="primary" size="mini" @click="addOrEdit('add')">新增</el-button><!--v-if="isShow"-->
        </div>
      </div>
    </div>
    <el-table
      ...
      <el-table-column align="center" v-if="isShow" prop="relationIns" label="与被保人关系" show-overflow-tooltip>
      ...
      <el-table-column align="center" v-if="isShow" prop="payeeMobile" label="手机号" show-overflow-tooltip/>
      <el-table-column align="center" prop="accAttribute" label="账户属性" show-overflow-tooltip>
        <template slot-scope="scope">
          <span v-if="isShow">{{ selectDictLabel(account_attributeOptions, '0') }}</span>
      ...
      <el-table-column align="center" v-if="isShow" label="操作" width="140">
        <template slot-scope="scope">
            ...
            <el-col :span="8">
              <el-form-item label="证件有效日期：" prop="idEndDate">
                <el-date-picker
                  ...
                  type="date"
                  placeholder="选择日期"/>
                <el-checkbox v-model="baseForm.checked" @change="">长期</el-checkbox>
              </el-form-item>
            </el-col>
          </el-row>
          <el-row>
              ...
              <el-form-item label="职业：" prop="payeeOccupation">
                <el-select v-model="baseForm.payeeOccupation" filterable class="item-width"
                  ...
                  <!-- <el-option v-for="option in payee_occupationOptions" :key="option.dictValue" :label="option.dictLabel"
                              :value="option.dictValue"/>-->
                  <el-option key="01" label="开发"
                             value="01"/>
                  <el-option key="02" label="测试"
                             value="02"/>
                </el-select>
              </el-form-item>
            </el-col>
...
<script>
  ...
  import {listRemark, addPayee, editPayee, delPayee} from '@/api/claim/handleCom'
  ...
  export default {
    ...
    methods: {
      add() {
        ...
        if (this.batchInfoData.claimtype === '01') {
          this.isShow = true
        }
      },
      save() {//新增  编辑
