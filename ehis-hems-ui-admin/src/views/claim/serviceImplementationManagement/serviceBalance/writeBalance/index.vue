...
<template>
  <div class="app-container">
    <el-card class="box-card">
        ...
        >
          ...
          <el-table-column label="操作" align="center" fixed="right" class-name="small-padding fixed-width" width="120px">
            <template slot-scope="scope">
              ...
              <el-button v-if="scope.row.bussinessStatus==='04'" size="mini" type="text" icon="el-icon-d-arrow-left" style="padding: 0px;" @click="handleWrite(scope.row)">核销</el-button>
            </template>
          </el-table-column>
        </el-table>
        <!-- 表格区域 END -->
      ...
      <!-- 核销弹窗 -->
      <el-dialog title="结算核销" :visible.sync="writeForm.visible" width="500px" append-to-body>
        <el-form ref="writeForm" :model="writeForm" :rules="writeForm.rules" label-width="125px">
          <el-form-item label="核销日期" prop="balanceDate">
            <el-date-picker
              class="item-width"
              v-model="writeForm.balanceDate"
              type="date"
              :picker-options="dateOptions"
              value-format="yyyy-MM-dd"
              placeholder="选择日期">
        ...
        <div slot="footer" class="dialog-footer">
          <el-button size="mini" type="primary" @click="writeSubmit">确 定</el-button>
          <el-button size="mini" @click="writeForm.visible = false">取 消</el-button>
        </div>
      </el-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      writeForm: {
        //核销校验
        rules: {
          balanceDate: [{required: true, message: "核销日期不能为空！", trigger: "blur"}]
        },
        //点击数据
        selectData: null,
        //弹窗显隐
  ...
  methods: {
    ...
    /** 结算核销 */
    handleWrite(row) {
      this.writeForm.selectData = row;
      this.writeForm.visible = true;
    },
    /** 核销提交 */
    writeSubmit() {
      this.$refs.writeForm.validate((valid) =>{
        if (valid){
          const params = {
            params: {type: "WRITE"},
            taskNo: this.writeForm.selectData.taskNo,
            balanceDate: this.writeForm.balanceDate
          };
          updateBalance(params).then(res => {
            if (res != null && res.code === 200) {
              this.writeForm.selectData = null;
              this.writeForm.visible = false;
              this.getList();
            }
          }).catch(res => {
            this.msgError('结算核销失败！')
          });
        }
      });
    },
