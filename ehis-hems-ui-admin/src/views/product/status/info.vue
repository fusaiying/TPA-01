    ...
    <el-row :gutter="10" class="mb8">
        <el-row :gutter="10" class="mt20" style="text-align: right;">
          <el-button
            ...
            size="mini"
            @click="submitUpdate"
            v-hasPermi="['product:definition:productupdate']"
          >产品更新</el-button>
          <el-button
            type="primary"
            icon="el-icon-view"
...
<script>
  import {getServiceType, getProductClass, getProductSubClass} from "@/api/product/definition";
  import {getServiceComNo, getServiceNetWorkList, getProvinceAll, getCityByProvince, getDistrictByCity, updateService, queryServiceOrgAndNetWork, getProductInfoById, queryServiceCity, productUpdate} from "@/api/product/info";
...
//           ]
    ...
    methods: {
      ...
      submitUpdate(){
        ...
        console.info("有进来啦");
        productUpdate(this.form).then(response => {
          if (response.code === 200) {
            this.form = response.data;
            this.msgSuccess("产品更新成功！");
          }else{
      ...
      close() {
        ...
        this.$router.push({ path: "/product/status", query: { t: Date.now()}})
      }
    }
  };
</script>
